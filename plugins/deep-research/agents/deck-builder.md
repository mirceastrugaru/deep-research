...
- `roadmap.md`: the research directions with their status, parents, and depth. Source for the method slide's glossary and the tree shape.
- `log.md`: per-round scores and Phase D notes. Source for the method slide's round columns, scores, and spawned-direction counts. To get a per-direction view, pivot log.md's per-round score lines by direction id.
...
```
 ...
 7. Traceable. Dedicated dense citation slides at the end —
    ...
    rounds, stances, scores and counts all trace to
    log.md and roadmap.md, with no invented run data.
...
Build the `props` by reading two files. Invent nothing:

- **`log.md`** holds, per round, a `## Round N` block with a `scores:` line (each entry is `d-XXXXXX/stance=score`, a `(FAIL)` marker on a hard-gate failure), a synthesis word count, and a Phase D note. Read every round block. Build the `rounds` array from these: each round lists its agent runs (one per `scores:` entry), with the round number, the stance, and the score. To present a per-direction view, group the score entries across rounds by their `d-` id. Each round's one-line `summary` note paraphrases that round's Phase D line (what the round found, how many children it spawned, any re-run flagged). Use a direction's short name from roadmap.md, not its `d-` id.
- **`roadmap.md`** gives, per direction, a `## d-XXXXXX: name` heading, a `note:` line, a `parent:` line, and a `depth:` line. For each direction write a one-line plain-English `question` (what that direction set out to answer, paraphrasing the `note`). If `parent:` names another direction, set the glossary entry's `parent` to that direction's name; otherwise leave `parent` empty (a seed direction).

The `summary` line states counts derived from the files: total worker reports, passes, and fact-check failures re-run. Every number must trace to the files.

If `log.md` is missing or empty, build the slide from `roadmap.md` alone; if neither is present, skip the method slide rather than inventing one, and say so in your return line.

## Build method

1. Read `synthesis.md`, `brief.md`, `evidence.md`, `roadmap.md`, `log.md`, and the kit file fully. All are absolute paths in your spawn prompt. If the kit will not read, STOP and report (see above).
...
6. Build the research-method slide with `pushMethod` from `log.md` and `roadmap.md`.

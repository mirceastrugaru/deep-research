...
- **Compared to what?** For any claim that asserts a magnitude — "too small", "too cheap", "huge", "tiny", "not enough to", "more than enough" — check that the baseline it is judged against is named and is the right baseline. A number standing alone supports no magnitude claim: "$33m is too small to rebuild X" needs the cost of rebuilding X *and* a check that nothing in the source set already supplies the denominator (a run-rate, a headcount, a revenue line, a comparable's spend). If the baseline is missing or wrong, the magnitude claim is unverified — it fails `correctness` until the comparison is made. Often the denominator is already in the source set and was simply never put next to the number; if so, put it there.
...
- `evidence` — every non-trivial claim has a specific, named, non-marketing source. A claim sourced ONLY to materials the subject of the research produced or supplied about itself (its deck, CIM, website, memo, datasheet) is the subject's CLAIM, not independent evidence: it may appear in the deliverable tagged as the subject's own assertion, but it fails `evidence` as the sole support for any load-bearing conclusion. The deliverable must not present the subject's say-so as if independently established.
...
- **Confidence is for facts; inferences get a likelihood and a flip condition.** Do not attach "high confidence" to an interpretation, especially one read off a source's *silence* (absence of a stated X is not evidence of Y). An inference is stated as a likelihood plus the single thing that would change it: "likely a wrapper; what would flip this is the management architecture spec or the current IT run-rate." Reserve confidence labels for claims that are checkable facts. A confidently-worded inference is the most dangerous line in the document — it reads like a fact and is not one.
- **Resolve; do not narrate the hedge.** Two-sided analysis names the one uncertainty the conclusion turns on and says what evidence would settle it — it does not repeat "the evidence cuts two ways" / "on one hand … on the other" as a substitute for resolving. If you find yourself announcing even-handedness, stop and instead state: here is the deciding question, here is which way the current evidence leans, here is what would settle it. Performative balance is a writing-bar failure, same as LLM filler.
...
**The spine test — the most important pass.** Name the single claim the synthesis's central conclusion rests on. Then ask one question: is that claim a *verified fact*, or an *inference chain*? Trace it. If the conclusion stands on a chain of inferences (deck language → a number → a comparison → "therefore X"), the conclusion is only as strong as the weakest link, and the document must say so — lead with the spine, state what it rests on, and name the one piece of evidence that would confirm or break it. A fluent, well-sourced, two-sided-sounding document whose central claim rests on an unverified inference is NOT converged, regardless of how many rounds remain or how complete coverage looks: open or re-run a direction aimed squarely at the spine. Two guards on this pass: (a) the spine must have been *adversarially tested* — if no worker this run attacked the load-bearing claim itself (only peripheral directions), assign an adversarial worker to the spine next round; attacking the periphery while the spine goes unchallenged is a failed round. (b) Do not let this pass turn the document timid: a *peripheral* unknown still gets a best-calibrated estimate or proxy, clearly labelled — "unknown, this is a DD ask" is the right answer only when the unknown IS the spine. Resolving nothing and listing open questions is as much a failure as overclaiming.

Write a short Phase D note into `log.md` for this round: recency flags raised, contradictions found and how reconciled, material omissions surfaced, reachable facts re-labelled or assigned, materiality and framing corrections made, and the spine — name the central conclusion's load-bearing claim, whether it is a verified fact or an inference chain, and whether it has been adversarially tested. If Phase D finds an unresolved recency flag, an unresolved contradiction, a reachable fact still wrongly deferred, or a spine that rests on an untested inference, the run has NOT converged regardless of coverage counts — a direction must be opened or re-run to close it.
...
- **Phase D was done** — recency checked, internal consistency checked, material omissions named, and the spine identified (central claim named, fact-or-inference traced, adversarially tested or assigned for next round); the Phase D note is in `log.md`.
- **No magnitude claim stands without its baseline**, and no inference wears a confidence label or a subject's-own-materials source as if it were independent fact.

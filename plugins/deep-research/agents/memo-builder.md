...
- **No `-ing` tack-ons that fake depth.** A trailing participle clause - "cutting cost, reflecting its discipline", "launched in 2019, underscoring its lead" - asserts significance the facts do not carry. It is the consultant document's way of sounding analytical without adding a fact. Cut it, or make it a real sentence that states the consequence and cites it.
- **Say what a thing IS - use "is/are/has", not "serves as / represents / functions as / stands as".** Consultant register avoids the plain copula to sound weightier. "Iccrea is the parent" beats "Iccrea serves as the parent"; "the platform has four modules" beats "the platform features four modules". The plain verb is shorter and the reader trusts it more.
- **One name per entity.** Pick one term for each company, person, or system and reuse it - do not cycle "the company / the firm / the group / the business" for a single entity to avoid repetition. To an expert reader the synonyms read as possibly-different entities, so the variation is a correctness risk, not just a style tic. Repetition of the right name is correct.

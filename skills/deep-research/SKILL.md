...
- `findings/round-N/` exists with fewer files than the worker count in `state.md` → that round's research is incomplete. Re-spawn the missing workers (each overwrites its own file — idempotent), then continue from the judge step.
